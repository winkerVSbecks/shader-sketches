...
const frag = glsl(/* glsl */ `#version 300 es
  ...
  float windowFrame(vec3 p) {
    float sa = 3.;
    float sb = sa-1.;
    float t = 0.15;
    vec3 a = vec3(0, sa, 0);
    vec3 b = vec3(-sb, sa * 0.5, 0);
    vec3 c = vec3(-sb, 1, 0);
  ...
  vec2 doModel(vec3 p) {
    float w = windowFrame(p);
    // window inner
    float f1 = udQuad(p, vec3(0,-3,0), vec3(0,3,0), vec3(-2,1,0), vec3(-2,-3,0)) - 0.1;
    float f2 = udQuad(p, vec3(0,-3,0), vec3(0,3,0), vec3(2,1,0), vec3(2,-3,0)) - 0.1;
    float f = min(f1, f2);

    float j = max(jali(p), f);
