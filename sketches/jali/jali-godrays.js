...
const frag = glsl(/* glsl */ `#version 300 es
  ...
  void main() {
    ...
    ro.xz *= rot(PI + PI*0.125 * sin(playhead * 2.*PI));

    vec3 rd = camera(ro, rt, screenPos, lensLength);

    vec3 lp = vec3(0, 0, -5);
    vec3 shadowPos = lp + normalize(ro-lp);
    ...
    if (t.x < 20.) {
      ...
      float power = clamp(dot(nor, l)*.5 + .5, 0., 1.);

      float falloff = min(1., 1./length(pos.xy));
      power *= falloff*falloff;
      color += power * foreground;
    }

    // center light
    float centerDist = dot(screenPos - lp.xy, screenPos - lp.xy) * 20./resolution.x;

    if (t.x <= -0.5) {
      float g = doModel(shadowPos).x;
      float light = smoothstep(0., .03, g);
      color += min(10., light*.02/max(centerDist, 1e-3)) * foreground;
    }
      // volumetric starburst
      float sb = max(0., doModel(normalize(rayPlane(ro, rd, vec3(0), normalize(ro)))).x);
      // sb *= 3.*smoothstep(-.2,.1, centerDist-.1);
      sb *= 1.*smoothstep(-.2,.1, centerDist-.1);
      color += sb * background;
...
function colors(minContrast = 1) {
  let palette = tome.get(); // lemon_citrus
