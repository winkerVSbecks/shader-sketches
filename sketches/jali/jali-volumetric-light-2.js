...
const frag = glsl(/* glsl */ `#version 300 es
  ...
  #define TAU 6.283185
  #define sampleCount 2 // use 200 for stills, 20 for video and 2 for live
  ...
  float mesh(vec3 q) {
    ...
    float ra = 0.9;
    float rb = 0.15;
    vec2 sc = vec2(sin(PI),cos(PI));
    p.x = abs(p.x);
    float d = ((sc.y*p.x>sc.x*p.y) ? length(p-sc*ra) : abs(length(p)-ra)) - rb;
