...
const frag = glsl(/* glsl */ `
  ...
  float map(vec3 p) {
    ...
    p.xy = rotate(p.xy, playhead * -1.505 *PI);

    // d = sdEquilateralTriangle(p.xy, .09) * .5;
    float t = PI * .25;
    vec2 cs = vec2(cos(t), sin(t));
    const float ra = 0.0625;
    const float th = 0.01;

    // distance
    d = sdRing(p.xy, cs, ra, th) * .5;

    // vec2 gv = fract(p.xy)-.5;
    // vec2 id = floor(p.xy);
    // float width = .1;
    // // if(n<.5) gv.x *= -1.;
    // float dd = abs(abs(gv.x+gv.y)-.5);
    // dd = length (gv-sign (gv.x+gv.y)*.5) - .5;
    // d = smoothstep(.01, -.001, abs(dd)-width);
