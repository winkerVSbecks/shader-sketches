...
const frag = glsl(/*glsl*/ `
  ...
  vec3 twist(vec3 p) {
    float t = 0.05 * wobbly(p.y, 2.*sin(playhead*PI));
    float c = cos(t*p.y+t);
    float s = sin(t*p.y+t);
    mat2 m = mat2(c,-s,s,c);
  ...
  vec2 doModel(vec3 p) {
    ...
    for(int i = -20; i < 20; i++) {
      float z = float(i)*0.3;
      float y = .5 * wobbly(z, 2.*sin(playhead*PI));
      // d = combine(d, sdSphere(p + vec3(0, y, z), 2.*sin(PI*playhead)), .5);
      d = combine(d, sdSphere(p + vec3(0, y, z), 1.+k), .5);
    }
  ...
  void main() {
    ...
    vec3  rayDirection = camera(rayOrigin, rayTarget, screenPos, .75);
    ...
    if (collision.x > -0.5) {
      ...
      vec3 baseColor = vec3(power, power, power) * vec3(.05, .0, .97) * pos.y;
